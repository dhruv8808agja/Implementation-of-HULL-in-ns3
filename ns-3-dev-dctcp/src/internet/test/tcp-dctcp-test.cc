...
TcpDctcpCodePointsTest::Tx (const Ptr<const Packet> p, const TcpHeader &h, SocketWho who)
{
  ...
  if (who == SENDER)
    {
      m_senderSent++;
      NS_LOG_DEBUG ("Sender sent " << m_senderSent << "; testCase " << +m_testCase);
    }
  else
    {
      m_receiverSent++;
      NS_LOG_DEBUG ("Receiver sent " << m_receiverSent << "; testCase " << +m_testCase);
    }
      ...
      if (m_testCase == 1)
        {
          if (m_senderSent == 1)
            {
              NS_TEST_ASSERT_MSG_EQ (found, true, "IP TOS tag not found");
...
TcpDctcpCodePointsTest::Rx (const Ptr<const Packet> p, const TcpHeader &h, SocketWho who)
{
  ...
  if (who == SENDER)
    {
      m_senderReceived++;
      NS_LOG_DEBUG ("Sender received " << m_senderReceived << "; testCase " << +m_testCase);
    }
  else
    {
      NS_LOG_DEBUG ("Receiver; testCase " << +m_testCase);
    }
